...
fn main() {
	...
	while !complete {
		...
		match op_res {
			...
			_ => { board = gen_empty_board(size); queens = Vec::new() },
		}
		...
		if (empty_squares.len() == 0) && (queens.len() as u8 == size) {
			...
			queen_position = pick_random_pos(&empty_squares)
				.unwrap_or(gen_random_pos(size));
		}
	}
...
fn fill_pos(board: &mut Board, (r, c): Position) -> Result<(), Position> {
	...
	match board[r as usize][c as usize] {
		Square::Empty | Square::Unattainable => board[r as usize][c as usize] = Square::Unattainable,
		_ => error = Some((r as u8, c as u8)),
	}
	match error {
		Some(pos) => Err(pos),
		...
		if row_index == r as usize {
			for col_index in 0..board[row_index].len() {
				match board[row_index][col_index] {
					Square::Empty | Square::Unattainable => board[row_index][col_index] = Square::Unattainable,
...
fn pick_random_pos(positions: &Vec<Position>) -> Result<Position, ()> {
	match positions.len() {
		...
		len => {
			...
			let mut rng = rand::thread_rng();
			Ok(positions[between.ind_sample(&mut rng)]) // Does this move the value (at index) out of positions?
